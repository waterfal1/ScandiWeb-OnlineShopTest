            ...
            <div key={element.id} className='attributes-columns'>
              ...
              .map((item, counter: number) => (goodsAmount[num][1][0][index] == counter
                  ? <SelectedAttributesHeader key={counter} attributeSelected={() =>
                      attributeSelected(num, index, counter)} value={item.value} displayValue={item.displayValue} />
                  : <NonSelectedAttributes index={index} ind={counter} value={item.value} attributeSelected={() =>
