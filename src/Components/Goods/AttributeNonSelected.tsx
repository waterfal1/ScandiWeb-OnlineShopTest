...
export default class AttributeName extends React.Component<{
  attributeNonSelected: (input1: number, input2: number, input3: number) => void, value: string, productIndex: number,
  attributeIndex: number, itemIndex: number}> {
  render() {
    const { productIndex, attributeIndex, itemIndex, value } = this.props;
    return (
      <div onClick={() => this.props.attributeNonSelected(productIndex, attributeIndex, itemIndex)}
