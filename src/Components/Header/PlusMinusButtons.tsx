  ...
  render() {
    ...
    return (
      <div className='cart-window-center-flex-element'>
        <button onClick={() => setAmountUp(goodsAmount[goodsNumber])} className='cart-window-counter-btn'>+</button>
        {goodsAmount[goodsNumber][2]}
        <button onClick={() => setAmountDown(goodsAmount[goodsNumber])} className='cart-window-counter-btn'>-</button>
      </div>
    )
  }
}
